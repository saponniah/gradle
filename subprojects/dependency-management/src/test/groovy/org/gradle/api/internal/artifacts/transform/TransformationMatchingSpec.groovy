...
class TransformationMatchingSpec extends Specification {
    ...
    def "chain contains its final step"() {
        ...
        expect:
        chain.contains(step2)
        !chain.contains(step1)
        !step1.contains(chain)
        !step2.contains(chain)
    ...
    def "longer chain contains shorter chain"() {
        ...
        expect:
        longChain.contains(subChain)
        !subChain.contains(longChain)
    }
    ...
    def "different chains do not contain each other"() {
        ...
        expect:
        !chain1.contains(chain2)
        !chain2.contains(chain1)
        !chain3.contains(chain2)
        !chain3.contains(chain1)
        !chain2.contains(chain3)
        !chain1.contains(chain3)
